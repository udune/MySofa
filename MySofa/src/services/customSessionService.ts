...
export const customSessionService = {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.response) {
          ...
          switch (status) {
            ...
            case 429:
              throw new Error(
                "너무 많은 세션 생성 요청입니다. 잠시 후 다시 시도해주세요."
              );
            case 500:
              throw new Error(
  ...
  // MyItem Session 생성
  async createMyItemSession(myitemId: string): Promise<CustomSessionResponse> {
    try {
      const response = await axiosInstance.post<CustomSessionResponse>(
        `/custom-session/myitem/${myitemId}`,
        {}
      );
      return response.data;
    } catch (error) {
      console.error("MyItem session creation failed:", error);
      if (axios.isAxiosError(error)) {
        const axiosError = error as AxiosError;
        if (axiosError.response) {
          const status = axiosError.response.status;
          switch (status) {
            case 400:
              throw new Error("유효하지 않은 내 아이템입니다.");
            case 401:
              throw new Error("로그인이 필요합니다.");
            case 403:
              throw new Error("해당 아이템에 접근 권한이 없습니다.");
            case 404:
            ...
            default:
              throw new Error(`세션 생성에 실패했습니다. (${status})`);
          }
        } else if (axiosError.request) {
          throw new Error("네트워크 연결을 확인해주세요.");
        }
